...
int blockchainpb(void * srcdata,uint32_t srcdatasize, char *srcdataname, uint32_t srcdatanamesize, 
		char *enckey,const uint8_t *prikey, uint32_t prikey_len, uint8_t *buffer, 
		uint32_t buffersize,uint32_t *messagesize, char *tx_idparm, uint32_t tx_idsize, 
		int64_t  timestampparm,char *idp,uint32_t idpsize)
{
	...
	/*       加密                   */
	printf("Start encrypting the raw data...\n");
	blockchainencrypt(enckey, srcdata, (void *)encdata, srcdatasize, &encdatasize);
	printf("Encrypt the data:ok\n");
	printf("Start building payload...\n");
	...
	uint8_t certhash[32]= {183,251,49,202,40,188,135,2,163,146,14,85,56,198,
				205,4,3,138,113,154,70,90,234,36,6,201,99,248,188,13,55,226};
	printf("Building other members...\n");
