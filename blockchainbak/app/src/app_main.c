...
int main(void)
{
    ...
    if (regstart==0)
        {
            printf("socket_client_v success\n");
        }
    else
        {
            printf("socket_client_v failed\n");
        }
...
/***********   blockchain 0 srcip 1 srcport 2 desip 3 desport 4   tx_idparm 5  timestampparm 6  idp  7  data  8                         *************************/
char *src=argv[8];
uint32_t srcsize=strlen(src);
const uint8_t prikey[]={0x7f, 0x0f, 0x57, 0xbf, 0xa5, 0x57, 0xfb, 0x30, 0x00, 0x27, 0x5f, 0xc7, 0xf6, 0x94, 0x90, 0xe9, 0xc8, 0xeb, 0xde, 0xf0, 0xa3, 0xbd, 0x90, 0xfc, 0xf2, 0xac, 0xa4, 0x71, 0x46, 0xa2, 0x72, 0x95};
...
blockchainpb((void *)src,srcsize, "Tempment", 8, key, prikey, prikey_len, buffer, buffersize, &messagesize, argv[5],tx_idsize, timestampparm,argv[7],idpsize);
